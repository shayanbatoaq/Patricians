...
export function PatAIPageClient() {
  ...
  function handleSuggestedPrompt(prompt: string) {
    setDraftPrompt((currentDraftPrompt) => ({
      value: prompt,
      id: (currentDraftPrompt?.id ?? 0) + 1,
    }));
  }
      ...
      <Container className="relative max-w-7xl max-sm:px-0 max-sm:pb-0 pb-8 sm:pb-12">
        <div className="grid gap-6 lg:grid-cols-[0.78fr_1.22fr] lg:items-stretch">
          <aside className="hidden min-h-0 flex-col justify-between rounded-[1.7rem] border border-[var(--border)] bg-white/76 p-6 shadow-[0_24px_80px_-48px_rgba(8,45,134,0.34)] backdrop-blur-xl lg:flex">
            <div>
              ...
              <p className="mt-3 max-w-sm text-sm leading-7 text-[var(--muted-foreground)]">
                ...
                pricing, timelines, AI automation, website assistants, websites,
                marketing, or mobile app development.
              </p>
            </div>
                ...
                <div className="mt-3 grid gap-2">
                  {suggestedPrompts.map((prompt) => (
                    <button
                      ...
                      className="rounded-[1rem] border border-[var(--border)] bg-[var(--surface-alt)] px-4 py-3 text-left text-sm leading-6 text-[var(--muted-foreground)] transition-all duration-200 hover:border-[var(--brand-300)] hover:bg-white hover:text-[var(--foreground)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--brand-300)]"
                    >
                      {prompt}
                    </button>
                  ))}
          ...
          <div className="min-h-0">
            <PatAIChatPanel mode="page" draftPrompt={draftPrompt} />
          </div>
        </div>
      </Container>
    </section>
